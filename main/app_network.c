...
// #define EXAMPLE_ESP_WIFI_SSID      "Thuc Coffee "
// #define EXAMPLE_ESP_WIFI_PASS      "18006230"
...
typedef struct app_network_ {
    bool is_using_wifi;
    bool is_connected;
    app_ppp_handle_t ppp_handle;
} app_network_t;

app_network_handle_t app_network_init(app_network_config_t* config) {
    app_network_handle_t handle = malloc(sizeof(app_network_t));
    handle->is_using_wifi = config->is_using_wifi;
    return handle;
}
...
{
    app_network_handle_t handle = (app_network_handle_t)arg;
...
void wifi_init_sta(app_network_handle_t handle)
{
...
void app_network_start(app_network_handle_t handle) {
    ...
    if (handle->is_using_wifi) {
        ESP_ERROR_CHECK(nvs_flash_init());
        ESP_ERROR_CHECK(esp_netif_init());
        ESP_ERROR_CHECK(esp_event_loop_create_default());

        wifi_init_sta(handle);
    } else {
        handle->ppp_handle = app_ppp_init(NULL);
    }
...
bool app_network_is_connected(app_network_handle_t handle) {
    ...
    if (handle->is_using_wifi) {
        ...
        return app_ppp_is_connected(handle->ppp_handle);
    }
}